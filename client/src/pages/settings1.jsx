...
import { toggleTheme, setFontSize } from "../features/themeSlice";
...
export default function Settings() {
  ...
  const { theme, fontSize } = useSelector((s) => s.theme);
  ...
  const handleSaveProfile = async () => {
    ...
    try {
      const res = await API.patch("/auth/profile", { name: profile.name.trim() });
      dispatch(loginSuccess({ token: localStorage.getItem("token"), user: res.data.user }));
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      // Server also bumps tokenVersion — re-save fresh token so current session stays alive
      if (res.data.token) {
        dispatch(loginSuccess({ token: res.data.token, user }));
  ...
  const handleLogoutEverywhere = async () => {
    try {
      ...
      // Log out this device too
      dispatch(logout());
      navigate("/login");
    } catch (err) {
      toast.error(err?.response?.data?.message || "Failed to logout everywhere");
    }
  };
  ...
  const handleDeleteAccount = async () => {
    ...
    if (typed !== "DELETE") return toast.error("Account deletion cancelled");
    try {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-zinc-400 mt-1">Manage your account and preferences</p>
      </div>
        ...
        <div className="flex-1 min-w-0">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input className={inputCls + " pl-9 opacity-60 cursor-not-allowed"} value={profile.email} disabled />
                  </div>
                  <p className="text-xs text-gray-400 dark:text-zinc-500 mt-1">Email cannot be changed</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-zinc-300 mb-1.5">Current Workspace</label>
                  <input className={inputCls + " opacity-60 cursor-not-allowed"} value={currentWorkspace?.name || "—"} disabled />
                </div>
              </div>
          ...
          {activeTab === "security" && (
            <div className={cardCls}>
              ...
              <p className="text-xs text-gray-500 dark:text-zinc-400 -mt-2">Changing password also logs out all other sessions.</p>
          ...
          {activeTab === "appearance" && (
            <div className={cardCls}>
              ...
              {/* Font Size — actually works via CSS variable */}
              <div className="border-t border-gray-100 dark:border-zinc-800 pt-5">
                ...
                <p className="text-xs text-gray-400 dark:text-zinc-500 mb-3">
                  Adjusts text size across the entire app instantly
                </p>
                <div className="flex gap-2">
                  {["Small", "Medium", "Large"].map((size) => (
                    <button
                      ...
                      className={`px-5 py-2 rounded-lg border-2 transition font-medium ${
                        fontSize === size
                    ...
                    >
                      {size}
                      {fontSize === size && <Check className="size-3 inline ml-1.5 mb-0.5" />}
                    </button>
                  ))}
                </div>
                {/* Live preview */}
                <div className="mt-4 p-4 rounded-xl border border-gray-200 dark:border-zinc-700 bg-gray-50 dark:bg-zinc-800/50">
                  <p className="text-xs text-gray-400 dark:text-zinc-500 uppercase tracking-wider mb-2">Preview</p>
                  <p className="text-gray-900 dark:text-zinc-100 font-medium">Project Dashboard</p>
          ...
          {activeTab === "notifications" && (
            <div className={cardCls}>
              ...
              <p className="text-xs text-gray-500 dark:text-zinc-400 -mt-2">Choose what you want to be notified about</p>
              ...
              <div className="space-y-1">
                {[
                  ...
                  <div key={key} className="flex items-center justify-between py-3.5 border-b border-gray-100 dark:border-zinc-800 last:border-0">
                    <div>
          ...
          {activeTab === "danger" && (
            <div className={cardCls + " border-red-200 dark:border-red-900"}>
              ...
              <p className="text-xs text-gray-500 dark:text-zinc-400 -mt-2">These actions are irreversible. Please proceed with caution.</p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-4 rounded-xl border border-orange-200 dark:border-orange-900 bg-orange-50 dark:bg-orange-500/5">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-zinc-400 mt-0.5">
                      Instantly invalidates all active sessions on every device
                    </p>
                  </div>
                  <button
                    onClick={handleLogoutEverywhere}
                    className="flex items-center gap-2 px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white text-sm rounded-lg transition flex-shrink-0 ml-4"
                  >
                ...
                <div className="flex items-center justify-between p-4 rounded-xl border border-red-200 dark:border-red-900 bg-red-50 dark:bg-red-500/5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm rounded-lg transition flex-shrink-0 ml-4"
                  >
